...
import { User, Megaphone, Building2, MapPin, Send, Laptop, Phone, Mail, Globe, CheckCircle } from "lucide-react";
...
export default function FinalizarPage() {
    ...
    const handleSend = async () => {
        ...
        if (result.success) {
            setSuccessData({ protocolo: result.protocolo! });
        } else {
                ...
                {successData && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white rounded-[40px] shadow-2xl max-w-lg w-full p-12 flex flex-col items-center text-center"
                        >
                            {/* Ícone de Sucesso */}
                            <div className="w-24 h-24 bg-verde-natureza rounded-full flex items-center justify-center mb-8 shadow-lg shadow-verde-natureza/30">
                                <CheckCircle className="w-14 h-14 text-white" strokeWidth={2.5} />
                            </div>

                            {/* Título */}
                            <h2 className="text-3xl font-bold text-grafite mb-6">
                            ...
                            <div className="bg-slate-50 rounded-2xl p-6 mb-10 border-l-4 border-verde-natureza">
                                <p className="text-lg font-semibold text-grafite mb-1">Obrigado!</p>
                                <p className="text-slate-500 font-medium">
                                    Sua manifestação foi recebida. Guarde seu número de protocolo para acompanhar o andamento.
                                </p>
                            </div>
                            ...
                            <button
                                onClick={() => router.push("/")}
                                className="w-full h-20 bg-grafite text-white rounded-full text-2xl font-bold flex items-center justify-center gap-3 shadow-xl hover:bg-grafite/90 transition-colors"
                            >
                                VOLTAR AO INÍCIO
                            </button>
                        </motion.div>
